      ...
      <div class="group bg-noir-900 border border-gold-500/20 rounded-2xl overflow-hidden hover:border-gold-400/60 transition duration-300 flex flex-col shadow-lg">
        ...
        <div class="p-3 sm:p-3.5 flex-1 flex flex-col justify-between space-y-2">
          <div>
            ...
            <h3 class="text-xs font-semibold text-slate-100 line-clamp-1 mt-1" title="${product.title}">${product.title}</h3>
          </div>
          <div>
          ...
          <button onclick="window.addToCart('${product.product_id}')" class="w-full bg-gradient-to-r from-gold-500 to-gold-400 hover:from-gold-400 hover:to-gold-300 text-noir-950 font-bold text-xs py-2 rounded-xl transition flex items-center justify-center gap-1.5 shadow active:scale-95">
...
export function selectMenu(menuId, submenuId, title) {
  ...
  document.getElementById('searchHeading').innerText = title || 'The Heritage Edit';
  const hero = document.getElementById('heroBannerSection');
  if (menuId || submenuId) hero?.classList.add('hidden');
